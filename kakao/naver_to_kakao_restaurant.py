...
for name in restaurant_names:
  ...
  try:
    ...
    restaurant_name = ''
    detail_link = ''
    kakao_url_csv_writer.writerow([id, restaurant_name, detail_link])
    id += 1
    continue
  ...
  review_score_element = driver.find_element(By.XPATH, "//em[contains(@data-id, 'scoreNum')]")
  rating_count_element = driver.find_element(By.XPATH, "//a[contains(@data-id, 'numberofscore')]")
  ...
  rating_count_text = ''
  if '건' in rating_count_element.text:
    rating_count_text = rating_count_element.text.replace('건', '').strip()

  operate_time_text = operate_time_element.text

  kakao_url_csv_writer.writerow([id, restaurant_name_element.text, detail_link])
  csv_writer.writerow([id, restaurant_name_element.text, category_element.text, number_of_review_element.text, address_element.text, review_score_element.text, rating_count_text, phone_number_element.text, operate_time_text])
  id += 1
